...
function ChangePrice({
  ...
  let getRecommnededPrice = async () => {
    let res = await MarketService.recomendedPrice(item.asset_id);
    ...
    if (res.status === 200) {
      ...
      // setgetPrice(res?.data?.data?.filter((i) => i?.asset_id == item?.asset_id)[0]
      // ?.recommended_price)
  ...
  return (
    <div className="djSell_box mb-4">
      <div className="djPro__img">
        ...
        <p>{item?.asset_details?.details?.name}</p>
      </div>
